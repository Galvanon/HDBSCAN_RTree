...
class Index:
    ...
    def get_statistics(self):
        """
        Get comprehensive statistics about the R-tree index.
        
        Returns
        -------
        dict
            Dictionary containing all available statistics:
            - reads: Number of node reads
            - writes: Number of node writes  
            - nodes: Total number of nodes
            - data: Number of data entries
            - splits: Number of node splits
            - hits: Cache/buffer hits
            - misses: Cache/buffer misses
            - adjustments: Number of node adjustments
            - query_results: Total query results returned
        ...
        stats = {
            'reads': core.rt.Index_GetReads(self.handle),
            'writes': core.rt.Index_GetWrites(self.handle),
            'nodes': core.rt.Index_GetNumberOfNodes(self.handle),
            'data': core.rt.Index_GetNumberOfData(self.handle),
            'splits': core.rt.Index_GetSplits(self.handle),
            'hits': core.rt.Index_GetHits(self.handle),
            'misses': core.rt.Index_GetMisses(self.handle),
            'adjustments': core.rt.Index_GetAdjustments(self.handle),
            'query_results': core.rt.Index_GetQueryResults(self.handle),
            'tree_height': core.rt.Index_GetTreeHeight(self.handle),
        }
        ...
        for level in range(height):
            ...
            result = core.rt.Index_GetNumberOfNodesInLevel(
                self.handle, 
                ctypes.c_uint32(level), 
                ctypes.byref(n_nodes)
            )
            if result == 0:  # RT_None == success
                nodes_per_level.append(n_nodes.value)
            else:
                # Handle error - could raise or just break
                break
        
        stats['nodes_per_level'] = nodes_per_level
...
class Property:
    ...
    def get_bulk_load_method(self) -> int:
        return core.rt.IndexProperty_GetBulkLoadMethod(self.handle)

    def set_bulk_load_method(self, value: int) -> None:
        if value not in (0, 1):
            raise RTreeError("bulk_load_method must be 0 (STR) or 1 (HDBSCAN)")
        return core.rt.IndexProperty_SetBulkLoadMethod(self.handle, value)

    bulk_load_method = property(get_bulk_load_method, set_bulk_load_method)
    """Bulk load method. 0 = STR (default), 1 = HDBSCAN"""
